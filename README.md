...
<details>
<summary><b>4. 草稿管理（持久化保存）</b></summary>

支持将图文内容保存为草稿，提供完整的草稿管理功能。

**核心特性：**

- ✅ **双重保存**：同时保存到小红书平台和本地备份
- ✅ **持久化存储**：使用固定的浏览器用户数据目录（`.browser-data/`），程序重启后草稿不丢失
- ✅ **本地备份**：额外保存到 `.drafts/` 目录，防止浏览器数据清除导致丢失
- ✅ **无数量限制**：本地备份无限制，不受小红书 100 篇草稿限制
- ✅ **完整管理**：支持列表、查看、删除和发布操作

**工作原理：**

1. **保存草稿**：内容自动保存到小红书创作中心（浏览器本地存储）
2. **本地备份**：同时备份到本地 `.drafts/` 目录（JSON 格式）
3. **持久化**：浏览器复用机制确保草稿始终可见
4. **管理草稿**：通过 MCP 工具管理本地备份

**草稿管理工具：**

- `save_to_draft` - 保存草稿（双重保存）
- `list_drafts` - 列出所有本地草稿
- `get_draft` - 查看草稿详情
- `delete_draft` - 删除本地草稿
- `publish_from_draft` - 从草稿发布

**草稿存储位置：**

- **小红书平台**：创作中心可见，可直接编辑和发布
- **本地备份**：`.drafts/` 目录，JSON 文件格式
